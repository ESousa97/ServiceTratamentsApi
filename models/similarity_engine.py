...
class SimilarityEngine:
    ...
    def calculate_column_similarities(self, df: pd.DataFrame, text_columns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for column in text_columns:
                texts = df[column].dropna().astype(str).head(100).tolist()
                if not texts:
                    continue
    ...
    def find_semantic_duplicates(self, texts: List[str], threshold: float = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Gera embeddings
            embeddings = self.embedding_engine.generate_embeddings_batch(texts_sample)
    ...
    def cluster_by_similarity(self, texts: List[str], eps: float = None, min_samples: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            for cluster_id, cluster_texts in clusters.items():
                cluster_indices = [i for i, lbl in enumerate(clustering.labels_) if lbl == cluster_id]
    ...
    def find_most_similar_texts(self, query_text: str, candidate_texts: List[str], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            candidate_embeddings = self.embedding_engine.generate_embeddings_batch(candidates_sample)
    ...
    def analyze_semantic_coherence(self, texts: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            distances = [1 - cosine_similarity([embedding], [centroid])[0][0] for embedding in embeddings_array]
    ...
    def _generate_similarity_recommendations(self, similarities: Dict[str, Any]) -> List[str]:
        ...
        if avg_score < 0.3:
            recommendations.append("Baixa similaridade entre colunas - dados parecem ser complementares")
        elif avg_score > 0.8:
            recommendations.append("Alta similaridade geral - muitas colunas contêm informações redundantes")
        else:
            recommendations.append("Similaridade moderada entre colunas - estrutura de dados equilibrada")
        return recommendations

# Instância global que será configurada se necessário
similarity_engine = None

def get_similarity_engine(embedding_engine: EmbeddingEngine = None):
    """Obtém a engine de similaridade, inicializando se necessário"""
    global similarity_engine
    if similarity_engine is None:
        engine = embedding_engine or get_embedding_engine()
        similarity_engine = SimilarityEngine(embedding_engine=engine)
